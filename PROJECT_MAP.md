...
- [ ] ⚠️ **`gpt-image2-1k` 上游故障,已实测** —— 任务能建但 `progress:0` 立即失败,
      上游原话「图片生成失败,请凭请求编号联系管理员查询」。同链路的
      `gpt-image2-4k` 完全正常(73 秒出真 3840×2160),故非本站配置问题。
      任务号 `aiimg_1789133167927_1e916b4b`,待报上游。建议先禁用该渠道
- [x] ~~确认失败是否扣费~~ **不扣**。实测六次失败,`logs.quota` 全为 0
